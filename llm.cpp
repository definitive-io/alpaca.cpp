...
int run_llm(int argc, char ** argv, gpt_params params) {
    ...
    while (remaining_tokens > 0) {
        ...
        if (embd_inp.size() <= input_consumed && !is_interacting) {
            ...
            if (!input_noecho && params.use_color && embd_inp.size() == input_consumed && !params.use_socket) {
                printf(ANSI_COLOR_RESET);
            }
        }
        ...
        if (params.use_socket && clientConnectionPtr) {
            for (auto id : embd) {
                ...
                memcpy(&(response_buf[n_written]), response_token.c_str(), response_token.size());
                n_written += response_token.size();
            }
